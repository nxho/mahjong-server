...
def init_tiles(room_id):
    ...
    logger.info('Initialized game tiles')
...
def deal_tiles(room_id):
    ...
    logger.info('Dealt tiles to players')

# Notify player to start turn
def start_turn(sid):
    logger.info(f'Starting {sid}\'s turn')
...
def update_opponents(room_id):
    ...
    for player_uuid in player_uuids:
        ...
        logger.info(f'sending update_opponents event to player_uuid={player_uuid} with opponents={opponents}')
    ...
    def _validate_payload_fields(func):
        ...
        def wrapper_validate_payload_fields(*args, **kwargs):
            ...
            if type(payload) != dict:
                logger.info(f'ERROR: expected type={dict}, found type={type(payload)}')
                return
            ...
            if fields_not_present:
                logger.info(f'ERROR: sid={sid} called event_handler="{func.__name__}" but missing fields={fields_not_present}')
                return
...
def enter_game(sid, payload):
    ...
    if num_of_players == 4:
        logger.info('Enough players have joined, starting game')
        init_tiles(sid)
        deal_tiles(sid)
...
def end_turn(sid, payload):
    ...
    if discarded_tile not in player_tiles:
        logger.info(f'ERROR: discarded_tile={discarded_tile} does not exist in sid={sid} tiles')
        return
